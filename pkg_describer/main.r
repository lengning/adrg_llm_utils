...
#' @description Minimal utilities for logging LLM API calls
#' @keywords internal

#' Log LLM call details (optional console output)
#'
...
log_llm_call <- function(prompt, model, response) {
  if (!isTRUE(getOption("pkg_describer.log_calls", FALSE))) {
  ...
  message("pkg_describer LLM call:", log_entry)
}
